...
fn main() {
    ...
    while is_running {
        ...
        let read_content = file_handler::prompt(&format!(
            "Do you want to read the content of {}? yes/no",
            user.file_name
        ));

        user.read = file_handler::confirm_action(&read_content);
        ...
        let should_continue = file_handler::prompt("Add more users? yes/no");

        if !file_handler::confirm_action(&should_continue) {
            println!("Bye!");
            is_running = false;
        }
    }
